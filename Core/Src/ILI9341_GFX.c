...
void ILI9341_unicode(uint16_t codePoint, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor)
{
	uint8_t fOffset, fWidth , fHeight, fBPL;
	const uint8_t *tempChar;
	...
	if(codePoint == 0x2103)
	{
		...
		uint8_t cIndex = degreeIndex + (1*fOffset);

		tempChar = &font[degreeIndex + 4];
		for(int j = 0; j < fHeight; j++)
		{
			for(int i = 0; i < fWidth; i++)
			{
				uint8_t z = tempChar[fBPL * i + ((j & 0xF8) >> 3)];
				uint8_t b = 1 << (j & 0x07);
				if((z & b) != 0x00)
				{
					ILI9341_DrawPixel(X + i, Y + j, color);
